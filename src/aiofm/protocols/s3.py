...
def _get_client(endpoint_url: str, region_name: str, access_key_id: SecretStr, secret_access_key: SecretStr,
...
class MinioProtocol(BaseProtocol):
    ...
    async def ls(self, path: str, pattern: str = None, *args, **kwargs) -> Sequence:
        self.client.list_objects()

        return tuple(item)

    @asynccontextmanager
    async def open(self, path: str, *args, **kwargs):
        mode = kwargs.pop('mode', args[0] if len(args) else 'r')
    ...
    async def cp(self, src_path: str, dst_path: str):
        src_path = PurePath(src_path)
        dst_path_is_dir = isinstance(dst_path, str) and (dst_path.endswith('/') or dst_path.endswith('\\'))
        dst_path = PurePath(dst_path)
        src_item = self._get_tree_item(self.tree, src_path)
